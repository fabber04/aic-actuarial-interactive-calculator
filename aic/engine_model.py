...
Usage
  ...
  Or: scripts\run_engine.bat (demo) / scripts\run_glm.bat (CSV pricing)
...
# Reserving lives in aic.core.reserving — re-exported here for compatibility.
from aic.core.reserving import ReservingModel, Triangle
...
# ─────────────────────────────────────────────────────────────────────────────
# 3. LOSS RESERVING — implemented in aic.core.reserving
#    (Triangle, ReservingModel imported above for backward-compatible API)
# ─────────────────────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1].lower() in ("verify", "--verify"):
        from aic.model_verification import main as verify_main

        sys.exit(verify_main())
    if len(sys.argv) > 1 and sys.argv[1].lower() in ("glm", "price"):
        from aic.fremtpl_glm import main as glm_main
