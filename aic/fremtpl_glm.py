...
def retrain_pricing_model(
    ...
    *,
    ...
    if profile.name == "portfolio":
        from aic.portfolio_motor import prepare_portfolio_df

        raw = prepare_portfolio_df(raw, mlc)
    elif profile.name == "claims":
        from aic.claims_us import prepare_claims_df
...
def _run_engine_command(argv: Sequence[str]) -> int:
    ...
    def _resolve_engine_paths(ns: argparse.Namespace) -> None:
        if ns.profile == "portfolio":
            from aic.portfolio_motor import PORTFOLIO_MODEL_DIR, PORTFOLIO_REJECTED, PORTFOLIO_STORE
            ...
            from aic.claims_us import CLAIMS_MODEL_DIR, CLAIMS_REJECTED, CLAIMS_STORE
...
def _run_portfolio_command(argv: Sequence[str]) -> int:
    import aic.portfolio_motor as pm
